...
class _PreComptaPageState extends State<PreComptaPage>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      bool isDebit = true; // true = débit, false = crédit
  ...
  Future<void> _deleteInvoice(Invoice inv) async {
    ...
    final confirm = await _confirmAction(t.delete, t.confirmDelete);
    if (confirm) {
  ...
  Future<void> _garderFnp(Invoice fnp) async {
    ...
    final copy = fnp.copyWith(id: "fnp-kept-${DateTime
        .now()
  ...
  Future<void> _exportFnpExcel(List<Invoice> items) async {
    ...
    var sheet = excel['FNP'];
    sheet.appendRow([
      ex.TextCellValue(t.supplier),
      ex.TextCellValue(t.description),
      ex.TextCellValue(t.account_label),
      ex.TextCellValue(t.ht),
      ex.TextCellValue(t.tva),
      ex.TextCellValue(t.ttc)
    ]);
    for (var i in items) {
      sheet.appendRow([
    ...
    await Share.shareXFiles([XFile(file.path)], text: t.export_fnp);
  }
  ...
  Widget _buildInvoiceList(InvoiceType type) {
    ...
    final bool isFrancePCG = _activeAccountingPlan == "France (PCG)";

    final filteredInvoices = _allInvoices.where((i) {
      final isSearchMatch = i.supplierOrClientName.toLowerCase().contains(
          _searchQuery.toLowerCase()) ||
          i.number.toLowerCase().contains(_searchQuery.toLowerCase());
      return !i.number.startsWith('FNP-') && i.type == type && isSearchMatch;
    }).toList();

    return Column(children: [
      if (type == InvoiceType.achat) Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
          child: ElevatedButton.icon(onPressed: _scanInvoice,
              icon: const Icon(Icons.document_scanner, color: Colors.black),
              label: Text(t.scanInvoiceBtn, style: const TextStyle(
                  color: Colors.black, fontWeight: FontWeight.bold)),
              style: ElevatedButton.styleFrom(backgroundColor: primaryColor,
                  minimumSize: const Size(double.infinity, 45),
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(10))))),
      Expanded(child: filteredInvoices.isEmpty ? Center(
          child: Text(t.noInvoiceFound)) : ListView.builder(
          ...
          itemBuilder: (ctx, index) {
            ...
            if (index == 0) {
              ...
              DateTime prevDate = filteredInvoices[index - 1].date;
              if (prevDate.month != inv.date.month ||
                  prevDate.year != inv.date.year) {
                showMonthDivider = true;
              }
            }
                  ...
                  decoration: BoxDecoration(
                      ...
                      border: Border.all(
                          color: isDark ? Colors.white10 : Colors.grey
                              .withAlpha(25), width: 1.2)),
                  child: Column(
                    children: [
                      if (isFrancePCG &&
                          inv.pdpStatus != PdpStatus.none) _buildPdpBadge(inv),
                      ListTile(
                        title: Row(mainAxisAlignment: MainAxisAlignment
                            .spaceBetween,
                            children: [
                              ...
                              Column(crossAxisAlignment: CrossAxisAlignment.end,
                                  children: [
                                    Text("${inv.amountTTC.toStringAsFixed(
                                        2)} ${inv.currency}", style: TextStyle(
                                        fontWeight: FontWeight.bold,
                                        fontSize: 12,
                                        color: type == InvoiceType.achat
                                            ? Colors.red[400]
                                            : Colors.green[600])),
                                    if (!isPaid && inv.totalPaid > 0) Text(
                                        "${t.rest}: ${inv.remainingAmount
                                            .toStringAsFixed(2)} ${inv
                                            .currency}", style: const TextStyle(
                                        fontSize: 10,
                                        color: Colors.orange,
                                        fontWeight: FontWeight.bold))
                                  ])
                          ...
                          Text(DateFormat('dd/MM/yyyy').format(inv.date),
                              style: TextStyle(fontSize: 11,
                                  color: isDark ? Colors.white70 : Colors
                                      .black54)),
                          const Spacer(),
                          _buildDynamicStatusBadge(inv)
                        ]),
                        trailing: PopupMenuButton<String>(onSelected: (
                            val) async {
                          if (val == 'view')
                            _showDetailedForm(type: type,
                                invoiceToEdit: inv,
                                isReadOnly: true);
                          else if (val == 'edit')
                            ...
                            _handlePrint(inv);
                          else if (val == 'send_pdp')
                            _sendViaPdp(inv);
                          else if (val == 'validate_pdp')
                            _updatePdpStatus(inv, PdpStatus.confirmed);
                          else if (val == 'reject_pdp') _updatePdpStatus(
                              inv, PdpStatus.rejected);
                        }, itemBuilder: (ctx) =>
                        [
                          _buildMenuItem(
                              'view', Icons.visibility_outlined, t.view),
                          if (!isPaid) _buildMenuItem(
                              'edit', Icons.edit_outlined, t.edit),
                          if (!isPaid &&
                              inv.status != InvoiceStatus.draft) _buildMenuItem(
                              'pay', Icons.account_balance_wallet_outlined,
                              type == InvoiceType.achat
                                  ? t.applyPaymentAction
                                  : t.collectAction),
                          if (isFrancePCG && type == InvoiceType.vente &&
                              (inv.status == InvoiceStatus.draft ||
                                  inv.pdpStatus ==
                                      PdpStatus.none)) _buildMenuItem(
                              'send_pdp', Icons.send_to_mobile,
                              "Envoyer via PDP", color: Colors.indigo),
                          if (isFrancePCG && type == InvoiceType.achat &&
                              inv.pdpStatus == PdpStatus.received) ...[
                            _buildMenuItem(
                                'validate_pdp', Icons.check_circle_outline,
                                "Valider PDP", color: Colors.green),
                            _buildMenuItem('reject_pdp', Icons.cancel_outlined,
                                "Rejeter PDP", color: Colors.red),
                          ],
                          _buildMenuItem(
                              'print', Icons.print_outlined, t.printAction),
                          _buildMenuItem(
  ...
  Widget _buildDynamicStatusBadge(Invoice inv) {
    final t = AppLocalizations.of(context);
    final isPaid = inv.isPaid || inv.status == InvoiceStatus.paid;
    final isPartiallyPaid = !isPaid && inv.totalPaid > 0.01;
    ...
    if (inv.type == InvoiceType.vente) {
      if (inv.status == InvoiceStatus.draft) return _buildStatusBadge(
          "BROUILLON", Colors.grey[700]!, Colors.grey[200]!);
      if (inv.pdpStatus == PdpStatus.pending) return _buildStatusBadge(
          "EN ATTENTE", Colors.orange[800]!, Colors.orange[50]!);
      if (inv.pdpStatus == PdpStatus.received)
        return _buildStatusBadge("REÇU", Colors.blue[800]!, Colors.blue[50]!);
      if (inv.pdpStatus == PdpStatus.confirmed && !isPaid)
        return _buildStatusBadge(
            "ACCEPTÉ - EN ATTENTE PAIEMENT", Colors.teal[800]!,
            Colors.teal[50]!);
      if (inv.pdpStatus == PdpStatus.rejected ||
          inv.status == InvoiceStatus.rejected)
        return _buildStatusBadge("REJETÉ", Colors.red[800]!, Colors.red[50]!);
    }

    // Workflow Standard / Achats
    if (isPaid) return _buildStatusBadge(
        t.paidStatus, Colors.teal[700]!, primaryColor.withOpacity(0.2));
    if (isPartiallyPaid)
      return _buildStatusBadge(t.partialStatus, Colors.white, Colors.orange);
    return _buildStatusBadge(
        inv.type == InvoiceType.achat ? t.toPayStatus : t.unpaidStatus,
        Colors.orange[800]!, Colors.orange[50]!);
  }

  Widget _buildPdpBadge(Invoice inv) {
    Color color = Colors.grey;
    String text = "PDP";
    switch (inv.pdpStatus) {
      case PdpStatus.pending:
        color = Colors.orange;
        text = "PDP : EN ATTENTE";
        break;
      case PdpStatus.received:
        color = Colors.blue;
        text = "PDP : REÇUE";
        break;
      case PdpStatus.confirmed:
        color = Colors.green;
        text = "PDP : CONFIRMÉE";
        break;
      case PdpStatus.rejected:
        color = Colors.red;
        text = "PDP : REJETÉE";
        break;
      default:
  ...
  Future<void> _sendViaPdp(Invoice inv) async {
    await _apiService.updateInvoice(inv.id, {
      'pdpStatus': 'pending',
      'status': 'pending',
      'pdpSentDate': DateTime.now().toIso8601String()
    });
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Facture envoyée via PDP avec succès"),
          backgroundColor: Colors.indigo));
      _loadData();
    }
  }

  Future<void> _updatePdpStatus(Invoice inv, PdpStatus status) async {
    await _apiService.updateInvoice(inv.id, {
      'pdpStatus': status
          .toString()
          .split('.')
          .last,
      if (status == PdpStatus.confirmed) 'status': 'validated'
    });
    if (mounted) {
      _loadData();
    }
  ...
  Widget _buildMonthDivider(DateTime date) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Text(
            DateFormat('MMMM yyyy', locale).format(date).toUpperCase(),
    ...
    if (result != null && result.files.single.path != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Row(children: [
            ...
            Text(t.localAnalysis)
            // On pourrait renommer en "Analyse par l'IA..."
          ]),
          backgroundColor: const Color(0xFF232435),
          duration: const Duration(seconds: 3)
      ));

      // --- CORRECTION ICI ---
      // On appelle la nouvelle méthode du backend au lieu de aiProvider
      final data = await _apiService.scanInvoiceWithAI(path);
      // ----------------------

      if (data.containsKey('error')) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(data['error']), backgroundColor: Colors.red));
        return;
      }

      // Le reste du code reste identique pour créer l'objet Invoice
      final scannedInvoice = Invoice(
          ...
          number: data['invoiceNumber'] ??
              "SCAN-${DateFormat('HHmm').format(DateTime.now())}",
          supplierOrClientName: data['supplierName'] ?? t.unknownSupplier,
          supplierOrClientId: "",
          amountHT: (data['amountTTC'] ?? 0.0) / 1.2,
          tva: (data['amountTTC'] ?? 0.0) * 0.2,
          amountTTC: data['amountTTC'] ?? 0.0,
          date: DateFormat('dd/MM/yyyy').tryParse(data['date'] ?? '') ??
              DateTime.now(),
          dueDate: null,
          type: InvoiceType.achat,
          entityId: _myEntities.isNotEmpty ? _myEntities.first.id : "",
          designation: data['designation'] ?? t.autoScanLabel);
  ...
  void _handlePayment(Invoice inv) {
    InvoiceDialogs.showPaymentDialog(context: context,
        invoice: inv,
        accounts: _accounts,
        onPaid: (bankId, date, amount, currency, rate) async {
          ...
          final bool isFullyPaid = (inv.amountTTC - newTotalPaid).abs() <
              0.05 || newTotalPaid >= inv.amountTTC;
          final p = Payment(id: DateTime
              .now()
              .millisecondsSinceEpoch
              .toString(),
              amount: amount,
          ...
          if (inv.linkedQuoteNumber != null &&
            ...
            final linkedQuote = _quotes.firstWhere((q) =>
            q['number'].toString() == inv.linkedQuoteNumber.toString(),
                orElse: () => {});
            if (linkedQuote.isNotEmpty) await _apiService.updateEstimateStatus(
                linkedQuote['id'], 'signé');
          }
  ...
  void _showDetailedForm(
    ...
    InvoiceDialogs.showInvoiceForm(context: context,
        type: type,
        ...
        quotes: _quotes,
        onSave: (inv) async {
          final bool exists = _allInvoices.any((item) => item.id == inv.id);
          if (exists)
            ...
            AlertDialog(title: Text(title),
                ...
                actions: [
                  ...
                  ElevatedButton(onPressed: () => Navigator.pop(ctx, true),
                      child: Text(t.validate.toUpperCase()))
                ])) ?? false;
  }
      ...
      appBar: AppBar(
          ...
          bottom: TabBar(controller: _tabController,
              ...
              tabs: [
                Tab(text: t.achats),
                Tab(text: t.ventes),
                Tab(text: t.journalEntries),
                Tab(text: t.fnp_label)
              ])),
        ...
        children: [
          ...
          Text(
            "${DateFormat('MMMM', locale)
                ...
                .toUpperCase()} - ${_focusedMonth.year}",
            style: TextStyle(
              color: primaryColor,
              fontWeight: FontWeight.bold,
              fontSize: 12,
    ...
    return Column(children: [
      Container(padding: const EdgeInsets.symmetric(vertical: 4),
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                _buildSubTabButton(t.invoices, 0),
                _buildSubTabButton(
                    type == InvoiceType.achat ? t.suppliers : t.customers, 1),
                if (type == InvoiceType.vente) _buildSubTabButton(t.quotes, 2)
              ])),
      Expanded(child: _partnersSubTab == 0
  ...
  Widget _buildSubTabButton(String label, int index) {
    ...
    return InkWell(onTap: () => setState(() => _partnersSubTab = index),
        child: Container(
            ...
            decoration: BoxDecoration(
                color: isActive ? primaryColor.withOpacity(0.3) : Colors
  ...
  Widget _buildJournalList() {
    ...
    final isDark = Theme
        .of(context)
        .brightness == Brightness.dark;
    final filteredEntries = _journalEntries
        .where((e) =>
    e.date.year == _focusedMonth.year && e.date.month == _focusedMonth.month)
        .toList();
    if (filteredEntries.isEmpty) return Center(child: Text(t.noEntryMonth));
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (index == 0) {
            ...
            DateTime prevDate = filteredEntries[index - 1].date;
            if (prevDate.month != entry.date.month ||
                prevDate.year != entry.date.year) {
              showMonthDivider = true;
            }
          }
            ...
            children: [
              ...
              Container(margin: const EdgeInsets.symmetric(vertical: 2),
                decoration: BoxDecoration(
                    color: isDark ? const Color(0xFF232435) : Colors.white,
                    borderRadius: BorderRadius.circular(12),
                    border: isDark ? Border.all(color: Colors.white10) : Border
                        .all(color: Colors.grey.shade200, width: 0.5)),
                child: ListTile(
                            ...
                            children: [
                              Text(DateFormat('dd/MM').format(entry.date),
                                  style: TextStyle(fontSize: 11,
                                      ...
                                      color: isDark ? Colors.white : Colors
                                          .black)),
                              Text(entry.journalNumber, style: TextStyle(
                                  fontSize: 9,
                                  color: isDark ? Colors.white54 : Colors
                                      .grey[600]))
                            ])),
                    Expanded(flex: 3,
                        child: Text(entry.lines.isNotEmpty ? entry.lines.first
                            .description : "-", style: TextStyle(
                            fontSize: 11, color: isDark
                            ? Colors.white70
                            : Colors.black87),
                            overflow: TextOverflow.ellipsis)),
                    Expanded(flex: 2,
                        child: Text(
                            "${entry.totalDebit.toStringAsFixed(2)} ${entry
                                .currency}", textAlign: TextAlign.right,
                            style: TextStyle(fontSize: 11,
                                fontWeight: FontWeight.bold,
                                color: isDark ? primaryColor : Colors
                                    .blueGrey))),
                  ]),
                  trailing: PopupMenuButton<String>(
                    ...
                    [
                      PopupMenuItem(
                          value: 'edit', child: Row(children: [const Icon(
                          Icons.edit_outlined, size: 18), const SizedBox(
                          width: 8), Text(t.edit)])),
                      PopupMenuItem(value: 'delete', child: Row(children: [
                        const Icon(
                            Icons.delete_outline, size: 18, color: Colors.red),
                        const SizedBox(width: 10),
                        Text(
                            t.delete, style: const TextStyle(color: Colors.red))
                      ])),
                    ],
                  ),
                ),
  ...
  void _viewEntry(JournalEntry entry) {
    InvoiceDialogs.showJournalEntryForm(context: context,
        entry: entry,
        isReadOnly: true,
        entities: _myEntities,
        accounts: _accounts,
        partners: [..._suppliers, ..._customers],
        costCenters: _costCenters,
        onSave: (_) {});
  }
  ...
  void _editEntry(JournalEntry entry) {
    InvoiceDialogs.showJournalEntryForm(context: context,
        ...
        entities: _myEntities,
        accounts: _accounts,
        // Ici, passe la liste globale, le dialogue filtrera
        partners: [..._suppliers, ..._customers],
        costCenters: _costCenters,
        onEntityChanged: (newEntityId) {
          // Cette fonction est appelée quand on change l'entreprise dans le formulaire
          final selectedEntity = _myEntities.firstWhere((e) =>
          e.id == newEntityId);
          print("Changement de plan comptable vers : ${selectedEntity
              .accountingPlan}");
          // Le dialogue va maintenant se reconstruire avec les comptes UK, FR, etc.
        },
  ...
  Future<void> _deleteEntry(String id) async {
    ...
    final confirm = await showDialog<bool>(context: context,
        builder: (ctx) =>
            AlertDialog(title: Text(t.delete),
  ...
  Widget _buildFnpView() {
    ...
    return Column(children: [
      Expanded(
          child: list.isEmpty ? Center(child: Text(t.noFnpMonth)) : ListView
              .builder(padding: const EdgeInsets.all(12),
              ...
              itemBuilder: (ctx, idx) {
                ...
                return Card(
                    ...
                    child: ListTile(title: Row(children: [
                      Expanded(child: Text(
                          item.supplierOrClientName, style: TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 13,
                          color: isDark ? Colors.white : Colors.black))),
                      if (isPaid) _buildStatusBadge(
                          t.paidStatus, Colors.teal[700]!,
                          primaryColor.withOpacity(0.2)) else
                        if (isKept) _buildStatusBadge(t.kept, Colors.blue[700]!,
                            Colors.blue[50]!) else
                          if (isExtourned) _buildStatusBadge(t.extourned,
                              Colors.grey[700]!, Colors.green[50]!) else
                            _buildStatusBadge(t.toPayStatus,
                                Colors.orange[800]!, Colors.orange[50]!)
                    ]),
                        subtitle: Text("${item.amountTTC.toStringAsFixed(2)} €",
                            style: TextStyle(
                                color: isDark ? Colors.white70 : Colors
                                    .black54)),
                        trailing: PopupMenuButton<String>(icon: Icon(
                            Icons.more_vert,
                            color: isDark ? Colors.white : Colors.grey),
                            onSelected: (v) {
                            ...
                            itemBuilder: (c) =>
                            [
                              PopupMenuItem(value: 'keep', child: Text(t.keep)),
                              PopupMenuItem(value: 'reverse', child: Text(t
                                  .reverse)),
                              PopupMenuItem(value: 'delete', child: Text(t
                                  .delete))
                            ])));
              })),
      Container(margin: const EdgeInsets.only(bottom: 80, left: 16, right: 16),
          ...
          child: Column(children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  _buildFnpStat(t.totalHT, "${totalHT.toStringAsFixed(2)} €"),
                  _buildFnpStat(t.totalTVA, "${totalTva.toStringAsFixed(2)} €"),
                  _buildFnpStat(t.totalTTC, "${totalTtc.toStringAsFixed(2)} €",
                      isMain: true)
                ]),
            ...
            SizedBox(width: double.infinity,
                child: OutlinedButton.icon(style: OutlinedButton.styleFrom(
                    ...
                    label: Text(t.provisionDetail, style: const TextStyle(
  ...
  void _showProvisionDetails(Map<String, double> summary, double totalHT,
    ...
    showModalBottomSheet(context: context,
        ...
        builder: (ctx) =>
            Container(color: isDark ? const Color(0xFF232435) : Colors.white,
                ...
                child: Column(mainAxisSize: MainAxisSize.min,
                    ...
                    children: [
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text(t.charge_detail_title, style: TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.bold,
                                color: isDark ? Colors.white : Colors.black)),
                      ...
                      const Divider(height: 24),
                      _buildDetailRow(t.totalHT, totalHT),
                      _buildDetailRow(t.totalTVA, totalTva),
                      _buildDetailRow(t.totalTTC, totalTtc, isMain: true),
                      ...
                      Row(children: [
                        ...
                        Expanded(child: ElevatedButton.icon(onPressed: () =>
                            _handlePrintFnp(
                                allFnp, totalHT, totalTva, totalTtc),
  ...
  Future<void> _passerOdProvision(List<Invoice> list) async {
    ...
    Map<String, double> groupedCharges = {};
    for (var f in list) {
      String acc = f.expenseAccount ?? '601000';
      groupedCharges[acc] = (groupedCharges[acc] ?? 0) + f.amountHT;
    }

    List<JournalLine> lines = [];
    double totalCredit4081 = 0;

    for (var entry in groupedCharges.entries) {
      lines.add(JournalLine(
          accountCode: entry.key,
          description: "Provision FNP - Compte ${entry.key}",
          debit: entry.value,
          credit: 0
      ));
      totalCredit4081 += entry.value;
    }

    lines.add(JournalLine(
        accountCode: '408100',
        description: "Contrepartie FNP Global",
        debit: 0,
        credit: totalCredit4081
    ));
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(t.od_generated), backgroundColor: Colors.green));
      _loadData();
    }
  }
      ...
      build: (pw.Context context) {
        return pw.Column(
          ...
          children: [
            pw.Header(level: 0,
                child: pw.Text(
                    "${t.fnp_state_title} - ${DateFormat('MMMM yyyy', locale)
                        .format(_focusedMonth)}")),
            pw.TableHelper.fromTextArray(data: [
              [t.supplier, t.account_label, t.ht, t.tva, t.ttc],
              ...items.map((i) =>
              [
            ...
            pw.Align(alignment: pw.Alignment.centerRight,
                child: pw.Text("${t.totalTTC} : ${tttc.toStringAsFixed(2)} €",
                    style: pw.TextStyle(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              ...
              Text('${value.toStringAsFixed(2)} €', style: TextStyle(
                  color: isMain ? primaryColor : (isDark ? Colors.white : Colors
                      .black),
  ...
  void _showNewPartnerForm(InvoiceType type) async {
    final partner = await InvoiceDialogs.showPartnerForm(
        context: context, type: type, accounts: _accounts, entities: _myEntities);
    if (partner != null) {
  ...
  Widget _buildPartnersList(InvoiceType type) {
    ...
    return ListView.builder(padding: const EdgeInsets.all(12),
        ...
        itemBuilder: (ctx, idx) {
          ...
          return Card(color: isDark ? const Color(0xFF232435) : Colors.white,
              child: ListTile(title: Text(partner.name, style: TextStyle(
                  color: isDark ? Colors.white : Colors.black,
                  fontWeight: FontWeight.bold)),
                  subtitle: Text(partner.email, style: TextStyle(
                      color: isDark ? Colors.white70 : Colors.black54)),
                  trailing: PopupMenuButton<String>(icon: Icon(Icons.more_vert,
                      color: isDark ? Colors.white : Colors.grey),
                      onSelected: (val) async {
                        ...
                        if (val == 'print_statement') await _printStatement(
                            partner, type);
                      },
                      itemBuilder: (ctx) =>
                      [
                        _buildMenuItem('edit', Icons.edit_outlined, t.edit),
                        _buildMenuItem('print_statement', Icons.print_outlined,
                            t.account_statement, color: Colors.indigo)
                      ])));
        });
  }
  ...
  Future<void> _handlePrint(Invoice inv) async {
    final t = AppLocalizations.of(context);
    final entity = _myEntities.firstWhere((e) => e.id == inv.entityId,
        orElse: () => _myEntities.first);
    final partner = (inv.type == InvoiceType.achat ? _suppliers : _customers)
        .firstWhere((p) => p.id == inv.supplierOrClientId, orElse: () =>
        Supplier(id: inv.supplierOrClientId,
            name: inv.supplierOrClientName,
            address: '',
            email: '',
            paymentTerms: t.thirtyDays,
    ...
    pdf.addPage(pw.Page(pageFormat: PdfPageFormat.a4,
        ...
        build: (pw.Context context) =>
            pw.Column(
              ...
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                  children: [
                    pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start,
                        children: [
                          pw.Text(entity.name, style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold, fontSize: 18)),
                          pw.Text(entity.address,
                              style: const pw.TextStyle(fontSize: 10)),
                          pw.Text("Email: ${entity.email}",
                              style: const pw.TextStyle(fontSize: 10)),
                          if (entity.phone != null) pw.Text(
                              "Tel: ${entity.phone}",
                              style: const pw.TextStyle(fontSize: 10)),
                          pw.Text("${t.siret} : ${entity.idNumber}",
                              style: const pw.TextStyle(fontSize: 10))
                        ]),
                    pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.end,
                        children: [
                          pw.Text(inv.type == InvoiceType.vente ? t.invoices
                              .toUpperCase() : t.received_purchase,
                              style: pw.TextStyle(
                                  fontWeight: pw.FontWeight.bold,
                                  fontSize: 24,
                                  color: PdfColors.blue800)),
                          pw.Text("N° ${inv.number}", style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold)),
                          pw.Text("${t.date_label}: ${DateFormat('dd/MM/yyyy')
                              .format(inv.date)}"),
                          if (inv.dueDate != null) pw.Text(
                              "${t.due_date}: ${DateFormat('dd/MM/yyyy').format(
                                  inv.dueDate!)}",
                  ...
                  children: [
                    pw.Container(width: 250,
                        ...
                        child: pw.Column(
                            crossAxisAlignment: pw.CrossAxisAlignment.start,
                            children: [
                              pw.Text(t.addressed_to, style: pw.TextStyle(
                                  fontSize: 10, color: PdfColors.grey700)),
                              pw.SizedBox(height: 4),
                              pw.Text(partner.name, style: pw.TextStyle(
                                  fontWeight: pw.FontWeight.bold,
                                  fontSize: 12)),
                              if (partner.address.isNotEmpty) pw.Text(
                                  partner.address,
                                  style: const pw.TextStyle(fontSize: 10)),
                              if (partner.email.isNotEmpty) pw.Text(
                                  "Email: ${partner.email}",
                                  style: const pw.TextStyle(fontSize: 10)),
                              if (partner.siret != null &&
                                  partner.siret!.isNotEmpty) pw.Text(
                                  "${t.siret}: ${partner.siret}",
                                  style: const pw.TextStyle(fontSize: 10))
                            ]))
                  ]),
              pw.SizedBox(height: 40),
              pw.TableHelper.fromTextArray(headerStyle: pw.TextStyle(
              ...
              pw.Align(alignment: pw.Alignment.centerRight,
                  child: pw.Container(width: 200,
                      child: pw.Column(children: [
                        pw.Row(mainAxisAlignment: pw.MainAxisAlignment
                            .spaceBetween,
                            children: [
                              pw.Text("${t.totalHT}:"),
                              pw.Text("${inv.amountHT.toStringAsFixed(2)} €")
                            ]),
                        pw.Row(mainAxisAlignment: pw.MainAxisAlignment
                            .spaceBetween,
                            children: [
                              pw.Text("${t.totalTVA}:"),
                              pw.Text("${inv.tva.toStringAsFixed(2)} €")
                            ]),
                        pw.Divider(),
                        pw.Row(mainAxisAlignment: pw.MainAxisAlignment
                            .spaceBetween,
                            children: [
                              pw.Text("${t.totalTTC}:",
                                  style: pw.TextStyle(fontWeight: pw.FontWeight
                                      .bold)),
                              pw.Text("${inv.amountTTC.toStringAsFixed(2)} €",
                                  style: pw.TextStyle(fontWeight: pw.FontWeight
                                      .bold, fontSize: 16))
                            ])
                      ]))),
              ...
              pw.Center(child: pw.Text(
                  "${t.printed_on} ${DateFormat('dd/MM/yyyy HH:mm').format(
                      DateTime.now())} - ${t.generated_by_software}",
  ...
  Future<void> _printStatement(dynamic partner, InvoiceType type) async {
    final t = AppLocalizations.of(context);
    try {
      // 1. Filtrer les factures du partenaire
      final partnerInvoices = _allInvoices.where((inv) =>
      inv.supplierOrClientId == partner.id && inv.type == type).toList();

      List<Map<String, dynamic>> transactions = [];

      // 2. Préparer les données (Factures et Paiements)
      for (var inv in partnerInvoices) {
        // La facture elle-même
        transactions.add({
          'date': inv.date,
          'type': type == InvoiceType.achat ? t.suppliers : t.customers,
          'ref': inv.number,
          'amount': inv.amountTTC,
          'isDebit': type == InvoiceType.vente
          // Vente = Débit client, Achat = Crédit fournisseur
        });

        // Les paiements associés
        if (inv.status == InvoiceStatus.paid || inv.totalPaid > 0) {
          transactions.add({
            'date': inv.reconciledDate ??
                inv.date.add(const Duration(hours: 1)),
            'type': 'Paiement',
            'ref': 'PAY-${inv.number}',
            'amount': inv.totalPaid,
            'isDebit': type == InvoiceType.achat
            // Paiement fournisseur = Débit, Paiement client = Crédit
          });
        }
      }
          ...
          [
                ...
                child: pw.Row(
                    ...
                    children: [
                      pw.Column(
                          crossAxisAlignment: pw.CrossAxisAlignment.start,
                          children: [
                            pw.Text(t.account_statement.toUpperCase(),
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 18)),
            ...
            pw.TableHelper.fromTextArray(
                ...
                data: transactions.map((tr) {
                  double debitValue = tr['isDebit'] ? tr['amount'] : 0;
                  double creditValue = !tr['isDebit'] ? tr['amount'] : 0;
                  ...
                  return [
                    ...
                    tr['type'],
                    tr['ref'],
                    debitValue > 0 ? "${debitValue.toStringAsFixed(2)} €" : "",
                    creditValue > 0
            ...
            pw.Align(
                ...
                child: pw.Column(
                    ...
                    children: [
                      pw.Text(
                          "${t.final_balance} ${runningBalance.toStringAsFixed(
                              2)} €",
                          style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold, fontSize: 16)),
                      pw.Text(
                          runningBalance >= 0 ? t.position_debit : t
                              .position_credit,
                          style: pw.TextStyle(
                              fontSize: 10,
                              fontWeight: pw.FontWeight.bold,
                              // CORRIGÉ : pw. ajouté ici
  ...
  Widget? _buildFab() {
    return FloatingActionButton(backgroundColor: primaryColor,
        ...
        onPressed: () {
          ...
          switch (_tabController.index) {
            ...
            case 2:
              ...
              break;
            case 3:
              InvoiceDialogs.showFnpForm(context: context,
                  entities: _myEntities,
  ...
  void _showNewQuoteForm(
    ...
    final t = AppLocalizations.of(context);
    Invoice? quoteObject;
    if (quoteToEdit != null) quoteObject = Invoice(
        id: quoteToEdit['id']?.toString() ?? '',
        number: quoteToEdit['number']?.toString() ?? '',
        date: quoteToEdit['date'] is DateTime ? quoteToEdit['date'] : DateTime
            .tryParse(quoteToEdit['date']?.toString() ?? '') ?? DateTime.now(),
        ...
        paymentTerms: quoteToEdit['paymentTerms']?.toString() ?? t.thirtyDays);
    showQuoteDialog(context: context,
  ...
  void _showJournalEntryForm() {
    InvoiceDialogs.showJournalEntryForm(context: context,
        ...
        partners: [..._suppliers, ..._customers],
        costCenters: _costCenters,
        onSave: (entry) async {// 1. On envoie l'écriture au backend
          await _apiService.createJournalEntry(entry);
          // 2. On recharge les données de la page pour voir la nouvelle ligne
          _loadData();
        },
        onEntityChanged: (selectedEntityId) {
          // 3. On trouve l'objet Entity complet correspondant à l'ID sélectionné
          final selectedEntity = _myEntities.firstWhere(
                (e) => e.id == selectedEntityId,
            orElse: () => _myEntities.first, // Par défaut si non trouvé
          );
        }
    );
  }
  ...
  void _deleteQuote(String id) async {
    ...
    final confirm = await showDialog<bool>(context: context,
        builder: (ctx) =>
            AlertDialog(title: Text(t.quote_delete_title),
                content: Text(t.quote_delete_msg),
                actions: [
                  TextButton(onPressed: () => Navigator.pop(ctx, false),
                      child: Text(t.cancel.toUpperCase())),
    ...
    return ListView.builder(padding: const EdgeInsets.all(12),
        ...
        itemBuilder: (ctx, index) {
          ...
          return Card(color: isDark ? const Color(0xFF232435) : Colors.white,
              ...
              child: ListTile(onTap: () => _showQuoteDetails(q),
                  ...
                  subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("${t.quotes} N° ${q['number']} - ${q['designation']
                            .toString()
                            .split('|')[0]}", style: TextStyle(
                            color: isDark ? Colors.white70 : Colors.black54)),
                        Text("${t.amount_label} : ${q['total']} €",
                            style: TextStyle(
                  ...
                  trailing: PopupMenuButton<String>(icon: Icon(Icons.more_vert,
                      ...
                      onSelected: (val) {
                        ...
                        if (val == 'print') _handlePrintQuote(q);
                      ...
                      [
                        _buildMenuItem(
                            'view', Icons.visibility_outlined, t.view),
                        _buildMenuItem(
                            'print', Icons.print_outlined, t.print_action),
                        _buildMenuItem('delete', Icons.delete_outline, t.delete,
                            color: Colors.red)
                      ])));
        });
  }
  ...
  Widget _buildQuoteStatusFromMap(Map<String, dynamic> quoteMap) {
    final t = AppLocalizations.of(context);
    final Invoice? linkedInvoice = _allInvoices.cast<Invoice?>().firstWhere((
        inv) => inv?.linkedQuoteNumber == quoteMap['number'].toString(),
        orElse: () => null);
    final bool hasLinkedInvoice = linkedInvoice != null;
    final bool isPaid = linkedInvoice?.status == InvoiceStatus.paid;
    final bool isPartiallyPaid = linkedInvoice != null && !isPaid &&
        linkedInvoice.totalPaid > 0.01;
    final String currentStatus = quoteMap['status']?.toString().toLowerCase() ??
        'brouillon';
    if (isPaid)
      return _badge(t.signed, Colors.green); else if (isPartiallyPaid)
      return _badge(t.partial_status, Colors.orange); else if (hasLinkedInvoice)
      return _badge(t.invoiced, Colors.blueGrey); else
      return Text(currentStatus.toUpperCase(),
  ...
  void _showQuoteDetails(Map<String, dynamic> quote) {
    ...
    if (!_customers.any((c) => c.id == customerId) &&
      ...
      _customers.add(Supplier(id: customerId,
    ...
    if (!_myEntities.any((e) => e.id.toString() == entityId) &&
        quote['entityName'] != null) {
      _myEntities.add(Entity(id: entityId,
  ...
  Future<void> _handlePrintQuote(Map<String, dynamic> quote) async {
    final t = AppLocalizations.of(context);
    final entity = _myEntities.firstWhere((e) =>
    e.id.toString() == quote['entityId']?.toString(), orElse: () =>
    _myEntities.isNotEmpty ? _myEntities.first : Entity(id: '',
        name: 'Émetteur inconnu',
        address: '',
        email: '',
        idNumber: ''));
    final client = _customers.firstWhere((c) =>
    c.id.toString() == quote['customerId']?.toString(), orElse: () =>
        Supplier(id: '',
        ...
        [
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
              children: [
                ...
                pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.end,
                    children: [
                      pw.Text(t.quotes.toUpperCase(), style: pw.TextStyle(
                          fontSize: 30,
                          fontWeight: pw.FontWeight.bold,
                      ...
                      pw.Text(
                          "${t.date_label} : ${DateFormat('dd/MM/yyyy').format(
          ...
          pw.SizedBox(height: 40),
          pw.TableHelper.fromTextArray(border: null,
                      ...
                      children: [
                        pw.Padding(
                            ...
                            child: pw.Text(title, style: pw.TextStyle(
                                fontWeight: pw.FontWeight.bold))),
                        if (longDescription.isNotEmpty) pw.Padding(
          ...
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.end,
              children: [
                pw.Container(width: 180,
                    child: pw.Column(children: [
                      _buildTotalRow(t.totalHT,
                          "${quote['totalHT'] ?? quote['amountHT']} €"),
                      _buildTotalRow(t.totalTVA, "${quote['tva'] ?? '0.00'} €"),
                      pw.Divider(color: PdfColors.grey400),
                      _buildTotalRow(
                          t.totalTTC, "${quote['total']} €", isBold: true)
                    ]))
              ]),
          pw.SizedBox(height: 50),
          pw.Text("${t.payment_terms_prefix} ${quote['paymentTerms'] ??
              t.thirtyDays}", style: const pw.TextStyle(fontSize: 9)),
          pw.Text(t.validity_quote, style: const pw.TextStyle(fontSize: 9)),
        ],
        footer: (pw.Context context) =>
            pw.Center(child: pw.Text(
                "Page ${context.pageNumber} / ${context.pagesCount} - ${t
                    .generated_by_software}", style: const pw.TextStyle(
                fontSize: 8, color: PdfColors.grey)))));
    await Printing.layoutPdf(onLayout: (format) async => pdf.save(),
        ...
        child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
            children: [
              pw.Text(label, style: pw.TextStyle(
                  fontWeight: isBold ? pw.FontWeight.bold : pw.FontWeight
                      .normal)),
              pw.Text(value, style: pw.TextStyle(
                  fontWeight: isBold ? pw.FontWeight.bold : pw.FontWeight
                      .normal, fontSize: isBold ? 14 : 10))
            ]));
  }
}