...
class _RhPageState extends State<RhPage> with SingleTickerProviderStateMixin {
  ...
  void _showEmployeeDetails(Employee e) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (context) => Container(height: MediaQuery.of(context).size.height * 0.8, decoration: BoxDecoration(color: Theme.of(context).scaffoldBackgroundColor, borderRadius: const BorderRadius.vertical(top: Radius.circular(30))), padding: const EdgeInsets.all(24), child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Center(child: Container(width: 40, height: 4, margin: const EdgeInsets.only(bottom: 20), decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(2)))), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('${e.firstName} ${e.lastName}', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)), _buildStatusChip(e)]), const SizedBox(height: 8), Text(e.post, style: TextStyle(fontSize: 16, color: Colors.grey[600], fontStyle: FontStyle.italic)), const Divider(height: 32), _buildDetailRow(Icons.business, 'Entité', _entities.firstWhere((ent) => ent.id == e.entityId, orElse: () => Entity(id: '', name: 'Inconnue', idNumber: '', email: '', address: '', phone: '')).name), _buildDetailRow(Icons.calendar_today, 'Embauche', DateFormat('dd/MM/yyyy').format(e.startDate)), _buildDetailRow(Icons.assignment_ind, 'Contrat', e.contractType.toString().split('.').last.toUpperCase()), _buildDetailRow(Icons.phone, 'Téléphone', e.phone.isEmpty ? 'Non renseigné' : e.phone), _buildDetailRow(Icons.home, 'Adresse', e.address.isEmpty ? 'Non renseignée' : e.address), _buildDetailRow(Icons.email, 'Email', (e.email == null || e.email!.isEmpty) ? 'Non renseigné' : e.email!), _buildDetailRow(Icons.family_restroom, 'Situation', e.maritalStatus), _buildDetailRow(Icons.contact_phone, 'Urgence', e.emergencyContact.isEmpty ? 'Non renseigné' : e.emergencyContact), const SizedBox(height: 24), const Text('Droits annuels au prorata (Workday style)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 12), _buildStatLine('Congés Payés', proratedVacation, _calculateTakenDays(e.id, 'Congé')), _buildStatLine('RTT', proratedRtt, _calculateTakenDays(e.id, 'RTT')), const SizedBox(height: 32), SizedBox(width: double.infinity, height: 50, child: OutlinedButton(onPressed: () => Navigator.pop(context), child: const Text('Fermer')))]))));
  }

  Widget _buildStatusChip(Employee e) {
    final bool isOnboarding = e.startDate.isAfter(DateTime.now()); Color color = e.isResigned ? Colors.red : (isOnboarding ? Colors.orange : Colors.green); String label = e.isResigned ? 'SORTI' : (isOnboarding ? 'À VENIR' : 'ACTIF');
  ...
  void _showOnboardingForm() {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (context) => StatefulBuilder(builder: (context, setM) => Container(height: MediaQuery.of(context).size.height * 0.9, decoration: BoxDecoration(color: Theme.of(context).scaffoldBackgroundColor, borderRadius: const BorderRadius.vertical(top: Radius.circular(30))), padding: const EdgeInsets.all(24), child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Center(child: Container(width: 40, height: 4, margin: const EdgeInsets.only(bottom: 20), decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(2)))), const Text('Nouveau Salarié (Intégration)', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)), const Divider(height: 32), _buildLabel('Entreprise d\'accueil *'), DropdownButtonFormField<String>(items: _entities.map((e) => DropdownMenuItem(value: e.id, child: Text(e.name))).toList(), onChanged: (v) => setM(() => selectedEntityId = v), decoration: InputDecoration(border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 24), _buildField(firstNameC, 'Prénom *'), _buildField(lastNameC, 'Nom *'), _buildField(postC, 'Poste *'), Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildLabel('Type de contrat'), DropdownButtonFormField<ContractType>(value: contract, items: ContractType.values.map((c) => DropdownMenuItem(value: c, child: Text(c.toString().split('.').last.toUpperCase()))).toList(), onChanged: (v) => setM(() => contract = v!), decoration: InputDecoration(border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))])), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildLabel('Date d\'arrivée'), OutlinedButton(style: OutlinedButton.styleFrom(minimumSize: const Size(double.infinity, 55), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), onPressed: () async { final d = await showDatePicker(context: context, initialDate: startD, firstDate: DateTime.now(), lastDate: DateTime(2100)); if (d != null) setM(() => startD = d); }, child: Text('${startD.day}/${startD.month}/${startD.year}'))]))]), const SizedBox(height: 32), SizedBox(width: double.infinity, height: 55, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Theme.of(context).brightness == Brightness.dark ? primaryColor : Colors.black, foregroundColor: Theme.of(context).brightness == Brightness.dark ? Colors.black : Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), onPressed: () async { if (firstNameC.text.isEmpty || lastNameC.text.isEmpty || selectedEntityId == null) return; final emp = Employee(id: DateTime.now().millisecondsSinceEpoch.toString(), firstName: firstNameC.text.trim(), lastName: lastNameC.text.trim(), post: postC.text.trim(), status: EmployeeStatus.salarie, contractType: contract, startDate: startD, address: '', phone: '', maritalStatus: 'Célibataire', emergencyContact: '', entityId: selectedEntityId); await _apiService.createEmployee(emp); _loadData(); if (context.mounted) Navigator.pop(context); }, child: const Text('Lancer l\'onboarding', style: TextStyle(fontWeight: FontWeight.bold))))])))));
  }
  ...
  void _showAddEmployeeForm({Employee? employeeToEdit}) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (context) => StatefulBuilder(builder: (context, setM) => Container(height: MediaQuery.of(context).size.height * 0.95, decoration: BoxDecoration(color: Theme.of(context).scaffoldBackgroundColor, borderRadius: const BorderRadius.vertical(top: Radius.circular(30))), padding: const EdgeInsets.all(24), child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Center(child: Container(width: 40, height: 4, margin: const EdgeInsets.only(bottom: 20), decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(2)))), Text(employeeToEdit == null ? 'Nouveau Salarié' : 'Modifier Salarié', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)), const Divider(height: 32), _buildLabel('Entité Juridique *'), DropdownButtonFormField<String>(value: selectedEntityId, items: _entities.map((e) => DropdownMenuItem(value: e.id, child: Text(e.name))).toList(), onChanged: (v) => setM(() => selectedEntityId = v), decoration: InputDecoration(border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 24), _buildField(firstNameC, 'Prénom *'), _buildField(lastNameC, 'Nom *'), _buildField(emailC, 'Email professionnel *'), _buildField(postC, 'Poste / Mission *'), Row(children: [Expanded(child: _buildField(vacationC, 'Droits Congés / an', keyboardType: TextInputType.number)), const SizedBox(width: 12), Expanded(child: _buildField(rttC, 'Droits RTT / an', keyboardType: TextInputType.number))]), Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildLabel('Statut'), DropdownButtonFormField<EmployeeStatus>(value: status, items: EmployeeStatus.values.map((s) => DropdownMenuItem(value: s, child: Text(s.toString().split('.').last.toUpperCase()))).toList(), onChanged: (v) => setM(() => status = v!), decoration: InputDecoration(border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))])), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildLabel('Contrat'), DropdownButtonFormField<ContractType>(value: contract, items: ContractType.values.map((c) => DropdownMenuItem(value: c, child: Text(c.toString().split('.').last.toUpperCase()))).toList(), onChanged: (v) => setM(() => contract = v!), decoration: InputDecoration(border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))]))]), const SizedBox(height: 24), Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildLabel('Date d\'embauche'), OutlinedButton(style: OutlinedButton.styleFrom(minimumSize: const Size(double.infinity, 55), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), onPressed: () async { final d = await showDatePicker(context: context, initialDate: startD, firstDate: DateTime(2000), lastDate: DateTime(2100)); if (d != null) setM(() => startD = d); }, child: Text('${startD.day}/${startD.month}/${startD.year}'))])), if (contract != ContractType.cdi) ...[const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildLabel('Date de fin *'), OutlinedButton(style: OutlinedButton.styleFrom(minimumSize: const Size(double.infinity, 55), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), onPressed: () async { final d = await showDatePicker(context: context, initialDate: endD ?? DateTime.now(), firstDate: DateTime(2000), lastDate: DateTime(2100)); if (d != null) setM(() => endD = d); }, child: Text(endD == null ? 'Choisir' : '${endD!.day}/${endD!.month}/${endD!.year}'))]))]]), const SizedBox(height: 24), _buildField(addressC, 'Adresse complète'), _buildField(phoneC, 'Téléphone perso', keyboardType: TextInputType.phone), _buildField(emergencyC, 'Contact urgence (Nom & Tél)'), const SizedBox(height: 32), SizedBox(width: double.infinity, height: 55, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: primaryColor, foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), onPressed: () async { if (firstNameC.text.isEmpty || lastNameC.text.isEmpty || selectedEntityId == null) return; final emp = Employee(id: employeeToEdit?.id ?? DateTime.now().millisecondsSinceEpoch.toString(), firstName: firstNameC.text.trim(), lastName: lastNameC.text.trim(), email: emailC.text.trim(), post: postC.text.trim(), status: status, contractType: contract, startDate: startD, endDate: endD, address: addressC.text.trim(), phone: phoneC.text.trim(), maritalStatus: maritalStatus, childrenCount: 0, emergencyContact: emergencyC.text.trim(), isResigned: employeeToEdit?.isResigned ?? false, yearlyVacationDays: double.tryParse(vacationC.text) ?? 25.0, yearlyRttDays: double.tryParse(rttC.text) ?? 5.0, entityId: selectedEntityId); if (employeeToEdit == null) { await _apiService.createEmployee(emp); } else { await _apiService.updateEmployee(emp.id, emp.toJson()); } _loadData(); if (context.mounted) Navigator.pop(context); }, child: Text(employeeToEdit == null ? 'Créer la fiche salarié' : 'Mettre à jour', style: const TextStyle(fontWeight: FontWeight.bold))))])))));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Theme.of(context).appBarTheme.backgroundColor, elevation: 0, leading: IconButton(icon: Icon(Icons.arrow_back_ios, color: isDark ? primaryColor : const Color(0xFF232435)), onPressed: () => Navigator.pop(context)), title: Text(t.rh, style: TextStyle(color: isDark ? Colors.white : Colors.black, fontWeight: FontWeight.bold)), bottom: TabBar(controller: _tabController, isScrollable: true, labelColor: isDark ? primaryColor : Colors.black, unselectedLabelColor: Colors.grey, indicatorColor: primaryColor, tabs: const [Tab(text: 'Salariés'), Tab(text: 'Absences'), Tab(text: 'Documents'), Tab(text: 'Titres-Resto')])),
  ...
  Widget _buildEmployeeList() {
    final hiredEmployees = _employees.where((e) => !e.startDate.isAfter(DateTime.now())).toList(); final onboardingEmployees = _employees.where((e) => e.startDate.isAfter(DateTime.now())).toList();
    return ListView(padding: const EdgeInsets.all(16), children: [if (hiredEmployees.isNotEmpty) ...[...hiredEmployees.map((e) => _buildEmployeeCard(e))], if (onboardingEmployees.isNotEmpty) ...[const Padding(padding: EdgeInsets.symmetric(vertical: 20), child: Row(children: [Expanded(child: Divider(color: Colors.grey, thickness: 0.5)), Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Text('BIENTÔT ARRIVÉS (ONBOARDING)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 1.2))), Expanded(child: Divider(color: Colors.grey, thickness: 0.5))])), ...onboardingEmployees.map((e) => _buildEmployeeCard(e, isOnboarding: true))], const SizedBox(height: 100)]);
  }

  Widget _buildEmployeeCard(Employee e, {bool isOnboarding = false}) {
    final isDark = Theme.of(context).brightness == Brightness.dark; double congTaken = _calculateTakenDays(e.id, 'Congé'); double rttTaken = _calculateTakenDays(e.id, 'RTT'); double proratedCongRights = _calculateProratedRights(e, e.yearlyVacationDays); double proratedRttRights = _calculateProratedRights(e, e.yearlyRttDays); final bool isInfoIncomplete = e.address.isEmpty || e.phone.isEmpty || e.emergencyContact.isEmpty;
    return Card(elevation: 0, margin: const EdgeInsets.only(bottom: 16), color: isDark ? const Color(0xFF232435) : Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: e.isResigned ? Colors.red.withAlpha(50) : (isDark ? Colors.white10 : Colors.black12))), child: ExpansionTile(leading: CircleAvatar(backgroundColor: e.isResigned ? Colors.grey[200] : (isOnboarding ? Colors.orange[50] : primaryColor.withAlpha(50)), child: Icon(Icons.person, color: e.isResigned ? Colors.grey : (isDark ? primaryColor : Colors.black))), title: Text('${e.firstName} ${e.lastName}', style: TextStyle(fontWeight: FontWeight.bold, decoration: e.isResigned ? TextDecoration.lineThrough : null, color: isDark ? Colors.white : Colors.black)), subtitle: Text(e.post, style: TextStyle(color: isDark ? Colors.white70 : Colors.black54)), trailing: PopupMenuButton<String>(onSelected: (v) async { if (v == 'view') _showEmployeeDetails(e); if (v == 'edit') _showAddEmployeeForm(employeeToEdit: e); if (v == 'toggle_resign') { await _apiService.updateEmployee(e.id, {'isResigned': !e.isResigned}); _loadData(); } }, itemBuilder: (ctx) => [const PopupMenuItem(value: 'view', child: Text('Voir la fiche')), const PopupMenuItem(value: 'edit', child: Text('Modifier')), PopupMenuItem(value: 'toggle_resign', child: Text(e.isResigned ? 'Réactiver' : 'Marquer démissionnaire'))]), children: [Padding(padding: const EdgeInsets.fromLTRB(16, 0, 16, 16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [if (isOnboarding) ...[const Text('🚀 PRÉPARATION ONBOARDING', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.orange)), const SizedBox(height: 12), ..._onboardingTasks.map((task) { final bool isDone = _completedTasks[e.id]?.contains(task) ?? false; return Padding(padding: const EdgeInsets.symmetric(vertical: 6), child: Row(children: [SizedBox(width: 24, height: 24, child: Checkbox(value: isDone, activeColor: Colors.orange, onChanged: (v) { setState(() { if (v == true) { _completedTasks.putIfAbsent(e.id, () => []).add(task); } else { _completedTasks[e.id]?.remove(task); } }); })), const SizedBox(width: 12), Expanded(child: Text(task, style: TextStyle(fontSize: 13, decoration: isDone ? TextDecoration.lineThrough : null, color: isDone ? Colors.grey : (isDark ? Colors.white70 : Colors.black87))))])); }), const Divider(height: 32)], _buildStatLine('Congés Payés', proratedCongRights, congTaken), const SizedBox(height: 4), _buildStatLine('RTT', proratedRttRights, rttTaken), if (isInfoIncomplete) ...[const SizedBox(height: 12), const Text('⚠️ Informations de contact à compléter', style: TextStyle(color: Colors.orange, fontSize: 11, fontStyle: FontStyle.italic))]]))]));
  }

  Widget _buildStatLine(String label, double? rights, double taken) {
    final isDark = Theme.of(context).brightness == Brightness.dark; String value = taken.toStringAsFixed(1); if (rights != null) value = "$taken pris / ${rights - taken} restants (Total proratisé: $rights)";
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text("${DateFormat('MMMM', 'fr_FR').format(current).toUpperCase()} - ${current.year}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
  ...
  Widget _buildAbsenceList() {
    ...
    return Column(children: [
      ...
      Expanded(child: filteredAbsences.isEmpty ? const Center(child: Text('Aucune absence pour ce mois.')) : ListView.builder(padding: const EdgeInsets.all(16), itemCount: filteredAbsences.length, itemBuilder: (context, index) { final a = filteredAbsences[index]; final emp = _employees.firstWhere((e) => e.id == a.employeeId, orElse: () => Employee(id: '', firstName: 'Inconnu', lastName: '', post: '', status: EmployeeStatus.salarie, contractType: ContractType.cdi, startDate: DateTime.now(), address: '', phone: '', maritalStatus: '', emergencyContact: '')); return Card(margin: const EdgeInsets.only(bottom: 12), color: isDark ? const Color(0xFF232435) : Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: BorderSide(color: isDark ? Colors.white10 : Colors.black12)), child: ListTile(title: Text('${emp.firstName} ${emp.lastName}', style: TextStyle(fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black)), subtitle: Text('${a.type} - ${a.isHalfDay ? "Demi-journée" : "${a.durationDays}j"}', style: TextStyle(color: isDark ? Colors.white70 : Colors.black54)), trailing: IconButton(icon: Icon(Icons.delete_outline, color: isDark ? Colors.redAccent : Colors.black), onPressed: () async { await _apiService.deleteAbsence(a.id); _loadData(); }))); }))
    ]);
  }
  ...
  Widget _buildField(TextEditingController c, String l, {TextInputType keyboardType = TextInputType.text}) { return Padding(padding: const EdgeInsets.only(bottom: 24), child: TextField(controller: c, keyboardType: keyboardType, decoration: InputDecoration(labelText: l, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))))); }
  ...
  Widget _buildDropdownField(String label, String value, List<String> items, bool enabled, Function(String?) onChanged) { return DropdownButtonFormField<String>(value: value, items: items.map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: enabled ? onChanged : null, decoration: InputDecoration(labelText: label, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))); }
}
